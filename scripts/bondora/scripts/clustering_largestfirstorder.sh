...
# process largest files first (assuming they will take the longest), good for low n_files_per_process
input_files=$(echo -n `ls -S1 ${DATA_DIR}/*.pndr | head -n ${i_end} | tail -n ${FILES_PER_PROCESS}` | sed 's/ /\:/g')
