...
""" Start - dataset and loader for optimal clustering parameters prediction """

class EmbeddingDataset(torch.utils.data.Dataset):
    def __init__(self, data_path, conf, max_events=None):
        self.file_paths = glob.glob(os.path.join(data_path, "*.pt"))
        if max_events is not None:
            self.file_paths = self.file_paths[:max_events]

        self.target_type = conf.target_type
        self.cheat = conf.CHEAT
        if self.cheat:
            logger.warning("This experiment will cheating!")
    ...
    def __getitem__(self, idx):
        t_data = torch.load(self.file_paths[idx])
        input, target_plateau_edges = t_data[0], t_data[1]
        if self.target_type == "midpoint":
            target = target_plateau_edges.mean(dim=1).unsqueeze(0)
        else:
            raise ValueError(f"target type '{self.target_type}' not valid")
        # XXX Cheating to check things are working
        if self.cheat:
            input[:, :64] = target[0][0]
            input[:, 64:] = target[0][1]
...
class CollateEmbeddings:
    ...
    def __call__(self, data_list):
        ev_clusters_embd = [ event_data["input"].unsqueeze(0) for event_data in data_list ]
        ev_clustering_target = torch.cat(
            [ event_data["target"].unsqueeze(0) for event_data in data_list ], dim=0
        )
        return {
            "ev_clusters_embd" : ev_clusters_embd, "ev_clustering_target" : ev_clustering_target
        }

""" End - dataset and loader for optimal clustering parameters prediction """

if __name__ == "__main__":
    from config_parser import get_config
    from model import ClusterMergeNet

    conf = get_config("experiments/test.yaml")
    model = ClusterMergeNet(conf, 1)
    model.eval()
    dataset = ClusterIterAugsDataset(os.path.join(conf.data_path, "train"), conf, 5, model, None)
