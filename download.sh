...
wget --no-check-certificate --load-cookies /tmp/cookies.txt "https://docs.google.com/uc?export=download&confirm=$(wget --quiet --save-cookies /tmp/cookies.txt --keep-session-cookies --no-check-certificate 'https://docs.google.com/uc?export=download&id=1A4za5GX8Y23kcUphMOofTDuOpAdvTOU6' -O- | sed -rn 's/.*confirm=([0-9A-Za-z_]+).*/\1\n/p')&id=1A4za5GX8Y23kcUphMOofTDuOpAdvTOU6" -O PandoraBdt_v08_33_00_SBND.xml && rm -rf /tmp/cookies.txt
